...
const URL_IMMAGINE_CARTA = `http://${SERVER_IP}/resources/img/Carte_Napoletane`;
...
export async function preparaTavolo() {
	...
	// console.log(`risultato mescola  =  ${risultatoMescola}`);
	if (risultatoMescola)
		carteATerra()
			.then(mazzo => {
				carte.push(...mazzo);
				// console.log("mazzo di carte   "+JSON.stringify(carte[0]));
			})
			.catch(errore => {
...
export function le_due_carte_sono_uguali(id1, id2) {
	...
	$(`.carte#${id1}, .carte#${id2}`)
		.attr("alt", "rimossa")
		.attr("src", `http://${SERVER_IP}:7003/resources/img/retro-carta-rimossa.jpg`);
}
...
export function le_due_carte_sono_diverse(id1, id2) {
	...
	$(`.carte#${id1}, .carte#${id2}`)
		.attr("alt", "coperta")
		.attr("src", `http://${SERVER_IP}:7003/resources/img/retro-carta.jpg`);
}

// 3. Mescola il mazzo
export async function mescola() {
	try {
		const mescolate = await $.post(`http://${SERVER_IP}:7003/mazzo/mescolaCarte`);
...
export async function carteATerra() {
	try {
		return await $.ajax({
			url: `http://${SERVER_IP}:7003/mazzo/carteATerra`,
			type: "GET",
			dataType: 'json'
		});
	} catch (errore) {
...
export async function giraCarta(carta) {
	...
	//console.log(`carta da girare ${carte[idCarta].percorso}`);
	
	carta.attr("alt", "girata");
	carta.attr("src", `http://${SERVER_IP}:7003/resources/img/Carte_Napoletane/${carte[idCarta].percorso}`);
	...
	for (let riga = 1; riga <= 4; riga++) {
		...
		for (let col = 1; col <= 10; col++) {
			...
			$("#riga" + riga).append(
				...
				"' src='" + "http://"+ SERVER_IP + ":7003/resources/img/retro-carta.jpg' alt='coperta' height='108' width='80' style='cursor:pointer;' />" +
				"</div>"
			);
		}
	}
