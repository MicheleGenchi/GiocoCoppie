...
async function mescola() {
    return $.post("http://localhost:7003/mazzo/mescolaCarte")
        .then(mescolate => {
    ...
    for (let riga = 1; riga <= 4; riga++) {
        ...
        for (let col = 1; col <= 10; col++) {
            ...
            $("#riga" + riga).append(
                ...
                "' src='http://localhost:7003/resources/img/retro-carta.jpg' alt='coperta' Height='108' width='80' />"
            );
        }
    }
...
function aggiungiGiocatori() {
	for (let i = 1; i <= numeroPartecipanti; i++) {
		...
		$.get("http://localhost:7002/giocatore/aggiungi/" + (i - 1) + "/" + nomeGiocatore);
	}
	return true;
}
...
function azzeraListaGiocatori() {
	var deferred = $.Deferred();
	$.get("http://localhost:7002/giocatore/azzeraLista", function (dati, status) {
		if (status === "success") {
...
async function giraCarta(carta) {
	...
	try {
		...
		await $.get("http://localhost:7003/mazzo/carta/" + idImage);
		return true;
	} catch (error) {
...
function gestisciTurno() {
    ...
    $.getJSON("http://localhost:7003/mazzo/confrontoCarte/" + idPrima + "/" + idSeconda)
...
function le_due_carte_sono_uguali(id1, id2) {
	...
	$.when(eliminaCarta(id1), eliminaCarta(id2))
		.done(function () {
			...
			$(".carte#" + id1).attr("src", "http://localhost:7003/resources/img/retro-carta-rimossa.jpg");
			$(".carte#" + id2).attr("alt", "rimossa");
			$(".carte#" + id2).attr("src", "http://localhost:7003/resources/img/retro-carta-rimossa.jpg");
			
            $("#puntiGiocatore").prop("disabled", false).val(punti).prop("disabled", true);
            
			$.get("http://localhost:7002/giocatore/aggiornaPunteggio/" + giocatore, function (dati) {
...
function aggiornaCampiGiocatore() {
	$.getJSON("http://localhost:7002/giocatore/cerca/" + giocatore, function (dati, status) {
		if (status == "success") {
...
function copriCarta(idCarta) {
	$(".carte#" + idCarta).attr("alt", "coperta").attr("src", "http://localhost:7003/resources/img/retro-carta.jpg");
}

// Completata la funzione che risultava troncata nel tuo frammento
function eliminaCarta(idxCarta) {
	var deferred = $.Deferred();
	$.get("http://localhost:7003/mazzo/eliminaCarta/" + idxCarta, function (dati, status) {
		if(status === "success") {
            deferred.resolve();
        } else {
            deferred.reject();
