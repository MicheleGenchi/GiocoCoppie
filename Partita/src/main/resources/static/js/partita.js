...
const BASE = "/"; //BASE = `http://${window.location.hostname}:8080`;
...
async function caricaModuliCarte() {
    try {
        ({
            ...
            attivaListenerCarte,
            preparaTavolo
        } = await import(`${SERVER_CARTE}/resources/js/carte.js`));
        console.log("Moduli carte remoti caricati con successo da:", SERVER_CARTE);
    } catch (error) {
...
async function gestisciTurno(carta1, carta2) {
	...
	if (confronto) {
		...
		aggiornaPunteggio(giocatoreCorrente,true);//true sta per aggiorna punteggio (default false)
...
function terminaPartita() {
	...
	setTimeout(() => {
		location.href = `${SERVER_PARTITA}/vittoria`;
	}, 2000);
}
...
$(document).ready(async function () {
	console.log("le costanti server");
	console.log(`SERVER_PARTITA => ${SERVER_PARTITA}`);
	console.log(`SERVER_CARTE => ${SERVER_CARTE}`);
	console.log(`SERVER_GIOCATORE => ${SERVER_GIOCATORE}`);
	console.log(`SERVER_INTESTAZIONE => ${SERVER_INTESTAZIONE}`);
